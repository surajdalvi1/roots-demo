...
errors =
  ...
  reserved for storing Contentstack system metadata, please rename this field to
  a different value.'
...
module.exports = (opts) ->
  ...
  if not (opts.api_key && opts.access_token && opts.environment)
  ...
  class RootsContentstack
    ...
    setup: ->
      validate_content(opts.content_types).with(@)
        .then(get_data)
        .tap(set_urls)
    ...
    validate_content = (ctypes) ->
      if _.isPlainObject(ctypes) then ctypes = reconfigure_alt_type_config(ctypes)
      W.map ctypes, (type) ->
        if not type.id then return W.reject(errors.no_type_id)
        type.filters ?= {}
        if (not type.name || (type.template && not type.path))
          return W client.contentType(type.id).then (res) ->
            type.name ?= pluralize(S(res.name).toLowerCase().underscore().s)
            console.log('type.name', type.name)
            if type.template
              type.path ?= (e) -> "#{type.name}/#{S(e[res.displayField]).slugify().s}"
            return t
        return W.resolve(t)
    ...
    reconfigure_alt_type_config = (ctypes) ->
